...
/**
 * A console menu for a grocery list (add, edit, remove, & display items) Note
 * that each string in file database.csv has to separated by a comma and a space
 * (", ") or the program cannot read from file database.csv
 * 
 * @author William McDaniel Albritton
 */
public class GroceryMenu2 {

	// data fields:
	// linked list of items
	private ListInterface<ComparableItem> groceryList = new OrderedLinkedList<ComparableItem>();
	...
	public static void main(String[] args) {
		// Cannot use driver program to test GroceryMenu2 class,
		// instead use database and user input.

		// Instantiate menu and display menu
		GroceryMenu2 menu = new GroceryMenu2();
		menu.displayLoop();
	}
	...
	public void displayLoop() {
		...
		// list of choices
		String[] choices = { "Add Item", "Edit Item", "Remove Item",
			...
			if (1 == theChoice) {
				try {
					...
					JOptionPane.showMessageDialog(null,
							"Unable to add item to grocery list.");
				}
			}
			// edit the list
			else if (2 == theChoice) {
				try {
					...
					JOptionPane.showMessageDialog(null,
							"Unable to edit item in grocery list.");
				}
			}
			// remove from list
			else if (3 == theChoice) {
				try {
					...
					JOptionPane.showMessageDialog(null,
							"Unable to remove item from grocery list.");
				}
			}
			// display the list
			else if (4 == theChoice) {
	...
	private void addToList() {
		...
		String numberString = JOptionPane
				.showInputDialog("Enter item's number");
		// convert string to integer
		int numberInteger = Integer.parseInt(numberString);
		// instantiate item and add to list
		ComparableItem groceryItem = new ComparableItem(name, numberInteger);
		groceryList.add(groceryItem);
	}

	/**
	 * Prompt user for item's position and item's number; and edit item in the
	 * list.
	 */
	private void editList() {
		// get user input
		String positionString = JOptionPane
				.showInputDialog("Enter item's position in the list");
		// convert from String to an Integer
		Integer positionInteger = Integer.parseInt(positionString);
		// get grocery item
		ComparableItem groceryItem = groceryList.get(positionInteger);
		// get item's number
		String numberString = JOptionPane
				.showInputDialog("Enter item's number");
		// convert from String to an Integer
	...
	private void readFromDatabase() throws FileNotFoundException {
		...
		while (fileReader.hasNextLine()) {
			...
			ComparableItem newItem = new ComparableItem(name, Integer.parseInt(number));
